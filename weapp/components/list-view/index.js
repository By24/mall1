...
var util = require('../../static/libs/util.js');
Component({
    /**
     * 组件的属性列表
     */
    ...
    ready: function ready() {
        let that = this;
        util.queryView(this, '.list-view-header', function(res) {
        ...
        that.setData({
            selfCustomStyle: _StyleHelper.getPlainStyle(that.data.customStyle),
        })
    },
    ...
    methods: {
        ...
        scrollBottomListener: function (e) {
            //加载更多
            if (this.data.state != 3 && this.data.state != 5 && this.data.length != null && this.data.length > 0 &&  this.data.loadMore && !this.data.noMore) {
                console.log('触发加载更多')
                this.data.scrollToTop = false;
